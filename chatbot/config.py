...
class Config:
    ...
    VK_SUPERUSER_ID = [(int(id) for id in environ.get('VK_SUPERUSER_ID').split()) if environ.get('VK_SUPERUSER_ID') is not None else 0]
    TG_ACCESS_TOKEN = environ.get('TG_ACCESS_TOKEN')
    TG_SUPERUSER_ID = [(int(id) for id in environ.get('TG_SUPERUSER_ID').split()) if environ.get('TG_SUPERUSER_ID') is not None else 0]
